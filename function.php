...
<div class="result"></div>';
	...
	$atts = shortcode_atts( array( "name"                      => '',
	                               ...
	                               "plugin_dir_loader_img_url" => plugins_url( 'images/loading.png', __FILE__ )
	), $atts );

	wp_enqueue_style( 'stylesheet', plugins_url( 'assets/css/far.css', __FILE__ ) );
	wp_register_script( 'far_admin_ajax', plugins_url( 'assets/js/far_admin.js', __FILE__ ), false );
	...
	if ( ! empty( $find_text ) && ! empty( $replace_text ) ) {
		...
		if ( ! empty( $results ) ) {
			foreach ( $results as $result ) {
				...
				$my_post       = array( 'ID'           => $result->ID,
				                        'post_title'   => $result->post_title,
				                        'post_content' => $rplace_string
				);
